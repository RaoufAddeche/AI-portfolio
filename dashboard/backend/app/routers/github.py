...
déclenchée à la demande (endpoint POST) ou par un cron externe / GitHub Action
qui appelle ce endpoint — aucun service LLM ne tourne en permanence.
"""
import asyncpg
...
async def sync_repos(
    ...
    limit: int = Query(10, ge=1, le=50, description="Nombre max de repos à résumer"),
    conn: asyncpg.Connection = Depends(get_db),
):
    """Synchroniser les repos GitHub : résumé LLM + upsert en draft dans portfolio_items.
    ...
    for r in candidates:
        try:
            ...
            synced.append(r["full_name"])
        except Exception as exc:  # noqa: BLE001
            errors.append({"repo": r["full_name"], "error": str(exc)})
